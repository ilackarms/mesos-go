...
func (driver *MesosSchedulerDriver) frameworkRegistered(from *upid.UPID, pbMsg proto.Message) {
	...
	if driver.status == mesos.Status_DRIVER_ABORTED {
		...
		fmt.Printf("ignoring FrameworkRegisteredMessage from master %s, driver is aborted\n", masterPid)
		return
	}

	if driver.connected {
		log.Infoln("ignoring FrameworkRegisteredMessage from master, driver is already connected", masterPid)
		fmt.Println("ignoring FrameworkRegisteredMessage from master, driver is already connected", masterPid)
		return
	}

	if driver.stopped() {
		log.Infof("ignoring FrameworkRegisteredMessage from master %s, driver is stopped", masterPid)
		fmt.Printf("ignoring FrameworkRegisteredMessage from master %s, driver is stopped\n", masterPid)
		return
	}
	if !driver.masterPid.Equal(from) {
		...
		fmt.Printf("ignoring framework registered message because it was sent from '%v' instead of leading master '%v'\n", from, driver.masterPid)
		return
	}

	log.Infof("Framework registered with ID=%s\n", frameworkId.GetValue())
	fmt.Printf("Framework registered with ID=%s\n", frameworkId.GetValue())
